		...
		try {
			...
			if (mCursor.moveToFirst()) {
				do {
					...
					if (!CommonStaticClass.dataId.substring(0, 2)
							.equalsIgnoreCase("00")
							&& (mCursor.getString(
									(mCursor.getColumnIndex("Qvar")))
									.equalsIgnoreCase("q15") || mCursor
									.getString(mCursor.getColumnIndex("Qvar"))
									.equalsIgnoreCase("q15family"))) {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case DELETEDONE:
				...
				CommonStaticClass.showMyAlert(con, "Completed", "ID deleted successfully");
